...
base_path = r"W:\Insync\GDrive\Main\TU Delft\Thesis\DRL runs 02"

# name_of_control_this_run = 'RLBaseNoForesight' 
list_names = ['RLBaseNoForesight', 'RL24hAllRNN', 'RL04hAllRNN']
...
for name_of_control_this_run in list_names:
    ...
    for builidng_enum_no, unique_building_name in enumerate(building_types_list):
        ...
        idf_edit_dates = IDF(fname1)
        idf_edit_dates.idfobjects['RUNPERIOD'][0].End_Month = 12
        idf_edit_dates.idfobjects['RUNPERIOD'][0].End_Day_of_Month = 31
        if name_of_control_this_run == 'EPBaseline': # 1 year only
            idf_edit_dates.idfobjects['RUNPERIOD'][0].End_Year = idf_edit_dates.idfobjects['RUNPERIOD'][0].Begin_Year
        else: # 3 years run period
            idf_edit_dates.idfobjects['RUNPERIOD'][0].End_Year = idf_edit_dates.idfobjects['RUNPERIOD'][0].Begin_Year + 2
        idf_edit_dates.save()
